										...
										<tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
											...
											<td
												style={{
													padding: "4px 8px",
													borderBottom: "1px solid #f3f4f6",
													color: c.awaited ? "#f59e0b" : "inherit",
												}}
											>
												{c.awaited ? "Awaited" : String(c.obtained)}
											</td>
											<td style={{ padding: "4px 8px", borderBottom: "1px solid #f3f4f6" }}>
												{String(c.weightedMax)}
											</td>
											<td
												style={{
													padding: "4px 8px",
											...
											>
												{c.awaited ? "Awaited" : String(c.weightedObtained)}
											</td>
										</tr>
									))}
								</tbody>
...
interface HeadCard {
	...
	email: string;
	phone: string;
	photo: string; // base64 src or empty
}

function parseHeadsHtml(html: string): HeadCard[] {
	const doc = new DOMParser().parseFromString(html, "text/html");
	const cards: HeadCard[] = [];

	// Each card is wrapped in <div class='item'>
	doc.querySelectorAll(".item").forEach((item) => {
		// Photo: base64 only (relative URLs like Dashboard/img/... won't load in extension)
		const imgSrc = (item.querySelector("img.card-img-top") as HTMLImageElement | null)?.getAttribute("src") ?? "";
		const photo = imgSrc.startsWith("data:image") ? imgSrc : "";

		// Role: text in .desig (the <i> icon has no text content so textContent is clean)
		const role = item.querySelector(".desig")?.textContent?.trim() ?? "";

		// Name + Designation: .col-md-12.text-small but NOT .text-muted (excludes school & contact)
		const infoEls = Array.from(item.querySelectorAll(".col-md-12.text-small:not(.text-muted)"));
		const name = infoEls[0]?.textContent?.trim() ?? "";
		const designation = infoEls[1]?.textContent?.trim() ?? "";

		// School: .text-muted.text-small (but not the contact block which has .h20)
...
function HeadsTab({ html }: { html?: string }) {
	if (!html) {
		return (
			<Section title="Know your Authorities">
				<p style={{ color: "#9ca3af", fontSize: 13 }}>No heads data — sync required</p>
			</Section>
		);
	}

	const cards = parseHeadsHtml(html);

	if (!cards.length) {
		// Fallback: just render the raw HTML with a bootstrap-like reset
		return (
			<Section title="Know your Authorities">
				<p style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>
					Could not parse cards — rendering raw HTML:
				</p>
				<div
					style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 6, padding: 12 }}
	...
	return (
		<Section title="Know your Authorities" count={cards.length}>
...
export default function DataViewer() {
			...
			<div style={rootStyle}>
				<div style={{ padding: 40, textAlign: "center" }}>
					...
					<p style={{ color: "#6b7280" }}>
						Open the extension popup and click <strong>Sync Now</strong> first (TEST MODE must be active).
					</p>
				</div>
			</div>
		);
	...
	return (
		<div style={rootStyle}>
			<style>{`
        .ums-ann-body { font-size: 12px; color: #4b5563; margin: 0; }
        .ums-ann-body p { margin: 0 0 4px; }
        .ums-ann-body p:empty, .ums-ann-body p:has(> br:only-child) { display: none; }
        .ums-ann-body ul, .ums-ann-body ol { margin: 2px 0 4px 16px; padding: 0; }
        .ums-ann-body li { margin: 1px 0; }
        .ums-ann-body a { color: #6366f1; }
      `}</style>
			{/* Header */}
			<div
				style={{
			...
			{data.studentInfo && (
				...
				>
					...
					<span>
						<strong>VID:</strong> {data.studentInfo.vid ?? "—"}
					</span>
					{data.apiData?.studentInfo?.RollNumber && (
						<span>
							<strong>Roll No:</strong> {data.apiData.studentInfo.RollNumber}
						</span>
					)}
					<span>
						<strong>Program:</strong> {data.studentInfo.program ?? "—"}
					</span>
					{data.apiData?.studentInfo?.Section && (
						<span>
							<strong>Section:</strong> {data.apiData.studentInfo.Section}
						</span>
					)}
					<span>
						<strong>CGPA:</strong> {data.studentInfo.cgpa ?? "—"}
					</span>
					{data.apiData?.studentInfo?.AggAttendance && (
						<span>
							<strong>Agg. Att:</strong> {data.apiData.studentInfo.AggAttendance}%
						</span>
					)}
					{data.apiData?.studentInfo?.PendingFee && (
						<span style={{ color: "#fca5a5" }}>
							<strong>Pending Fee:</strong> ₹{data.apiData.studentInfo.PendingFee}
						</span>
					)}
				</div>
			)}
			...
			<div style={{ padding: 24, maxWidth: 1200, margin: "0 auto", width: "100%" }}>
					...
					<div>
						...
						>
							{[
								...
								{ label: "Assessments", value: data.courseAssessments.length, color: "#8b5cf6" },
								...
								{ label: "Timetable Slots", value: data.timetable.length, color: "#f59e0b" },
								{
									...
									value: data.apiData?.courses?.length ?? 0,
									color: "#ec4899",
								},
								{
									label: "Announcements",
									value: data.apiData?.announcements?.length ?? 0,
									color: "#f97316",
								},
								{
									label: "Seating Plan",
									value: data.apiData?.seatingPlan?.length ?? 0,
									color: "#ef4444",
								},
								{ label: "Messages", value: data.apiData?.messages?.length ?? 0, color: "#64748b" },
								{ label: "Heads", value: data.apiData?.headsHtml ? "✓" : "—", color: "#0d9488" },
							].map((stat) => (
								<div
									key={stat.label}
									style={{
					...
					<div>
						{termsWithCourses.length > 0 ? (
							termsWithCourses.map((term) => (
								<Section
									key={term.id}
									title={`${term.displayName || term.id}${term.tgpa ? ` — TGPA: ${term.tgpa}` : ""}`}
						...
						{data.attendance.length === 0 ? (
							...
							<div>
								<table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
									<thead>
										<tr>
											{["Course Code", "Course Name", "Total", "Present", "Attendance %"].map(
												(h) => (
													<th
														key={h}
														style={{
				...
				{tab === "exams" && (
					<div>
						<Section title="Component Assessments" count={data.courseAssessments.length}>
							<ComponentWiseMarks assessments={data.courseAssessments} terms={termsWithCourses} />
						</Section>
						<Section title="Exam Marks (raw)" count={data.examMarks.length}>
							{data.examMarks.length === 0 ? (
								<p style={{ color: "#9ca3af", fontSize: 13 }}>
									No exam marks data (usually empty — superseded by component assessments)
								</p>
							) : (
								<Table
									headers={[
				...
				{tab === "courses" && (
					<Section title="Current Semester Courses (API)" count={data.apiData?.courses?.length ?? 0}>
						<Table
							...
							rows={(data.apiData?.courses ?? []).map((c) => [
								c.CourseCode,
								c.CourseName,
								c.Term,
								c.RollNo,
								...
								{(data.apiData?.announcements ?? []).map((a, i) => (
									...
									>
										...
										>
											...
											<Badge color="#6366f1">
												{CATEGORY_NAME[a.categorycode] ?? a.categorycode}
											</Badge>
										...
										<p style={{ fontSize: 11, color: "#6b7280", margin: "0 0 4px" }}>
											By: {a.employeename}
											{a.uploadedby && a.uploadedby !== a.employeename
												? ` (${a.uploadedby})`
												: ""}
										</p>
										<div
											className="ums-ann-body"
											dangerouslySetInnerHTML={{ __html: a.announcement ?? "" }}
										/>
										{Array.isArray(a.Files) && a.Files.length > 0 && (
											<div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
												{a.Files.map(
													(f: { FileName: string; filepath: string }, fi: number) => (
														<a
															key={fi}
															href={`https://ums.lpu.in${f.filepath}`}
															target="_blank"
															rel="noreferrer"
															style={{
																fontSize: 11,
				...
				{tab === "seating" && (
					<Section title="Exam Seating Plan" count={data.apiData?.seatingPlan?.length ?? 0}>
						<Table
							...
							rows={(data.apiData?.seatingPlan ?? []).map((s) => [
								s.CourseCode,
								s.CourseName,
								s.ExamDate,
								s.ExamType,
				...
				{tab === "raw-html" && (
					<div>
						{data.apiData?._rawHtml ? (
							Object.entries(data.apiData._rawHtml).map(([key, html]) => (
								<Section key={key} title={`Raw HTML: ${key}`}>
									{html ? (
										<pre
											style={{
												background: "#111827",
										...
										<p style={{ color: "#9ca3af", fontSize: 13 }}>Empty response</p>
									)}
								</Section>
							))
						) : (
							<p style={{ color: "#9ca3af", fontSize: 13 }}>
								Raw HTML only available after a sync with DEV_MODE=true
							</p>
						)}
					</div>
				)}
