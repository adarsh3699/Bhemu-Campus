...
const ChatMessageBubble = memo(function ChatMessageBubbleView({
	...
	const reactions = useMemo(() => summarizeChatReactions(message.reactions, currentUserId), [currentUserId, message.reactions]);
	...
	return (
		<GestureDetector gesture={swipeGesture}>
			<View style={local.swipeContainer}>
				<Animated.View pointerEvents="none" style={[local.replyAction, local.leftReplyAction, leftReplyActionStyle]}>
					<Reply size={18} color={Colors.primary} />
				</Animated.View>
				<Animated.View pointerEvents="none" style={[local.replyAction, local.rightReplyAction, rightReplyActionStyle]}>
					<Reply size={18} color={Colors.primary} />
				</Animated.View>
				<Animated.View style={[local.row, isOwn && local.ownRow, showIdentity ? local.groupStart : local.groupContinuation, swipeStyle]}>
					{!isOwn ? (
						<View style={local.avatarSlot}>
							{showIdentity ? (
								<View
									...
									style={[local.avatar, { backgroundColor: AVATAR_COLORS[getChatAvatarIndex(message.authorUid, AVATAR_COLORS.length)] }]}
								>
									<Text style={local.avatarText}>{getChatAuthorInitials(authorName)}</Text>
								</View>
							) : null}
					...
					<View style={[local.messageColumn, isOwn && local.ownMessageColumn]}>
						...
						<Pressable
							...
							style={[local.bubble, isOwn ? local.ownBubble : local.otherBubble, reactions.length > 0 && local.bubbleWithReactions, isPending && local.pendingBubble]}
						>
							{message.replyToMessageId ? (
								<View style={[local.replyPreview, isOwn ? local.ownReplyPreview : local.otherReplyPreview]}>
									<Text style={[local.replyAuthor, isOwn && local.ownReplyText]}>
										{repliedMessage ? (repliedMessage.authorUid === currentUserId ? "You" : normalizeChatDisplayName(repliedMessage.authorName)) : "Replied message"}
									</Text>
							...
							<View style={[local.footer, reactions.length > 0 && local.footerWithReactions]}>
									...
									<View style={local.reactions}>
										{reactions.map(([emoji, summary]) => (
											<Pressable
												...
												accessibilityLabel={summary.hasReacted ? `Remove ${emoji} reaction` : `React with ${emoji}`}
												hitSlop={4}
												onPress={() => summary.hasReacted ? onUnreact(message.id) : onReact(message.id, emoji)}
												style={({ pressed }) => [local.reaction, isOwn ? local.ownReaction : local.otherReaction, summary.hasReacted && local.reactionSelected, pressed && local.pressed]}
											>
								...
								<View style={local.meta}>
									{message.editedAt ? <Text style={[local.metaText, isOwn && local.ownMeta]}>edited</Text> : null}
									{message.failed ? (
										<Pressable accessibilityRole="button" accessibilityLabel="Retry sending message" onPress={() => onRetry(message.id)} style={local.retry}>
											<AlertCircle size={12} color={Colors.destructive} />
											<Text style={local.retryText}>Retry</Text>
										</Pressable>
									) : (
										<>
											<Text style={[local.metaText, isOwn && local.ownMeta]}>{formatChatTime(messageTimestamp(message))}</Text>
											{isOwn ? (isPending ? <Clock3 size={12} color={Colors.textPrimary} /> : <Check size={13} color={Colors.textPrimary} />) : null}
										</>
									)}
								</View>
							</View>
...
const local = StyleSheet.create({
	...
	author: { marginBottom: 6, paddingHorizontal: Spacing.xs, fontSize: FontSize.md, fontWeight: FontWeight.semibold, letterSpacing: -0.2, color: Colors.secondary },
	bubble: { minWidth: 80, paddingHorizontal: Spacing.sm, paddingVertical: Spacing.xs, borderWidth: 1, borderRadius: Radius.lg, borderCurve: "continuous", shadowColor: "#000000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.16, shadowRadius: 10, elevation: 2 },
	bubbleWithReactions: { minWidth: 116 },
	otherBubble: { borderColor: Colors.borderLight, borderBottomLeftRadius: Radius.sm, backgroundColor: Colors.surfaceElevated },
	...
	replyPreview: { marginBottom: Spacing.xs, minWidth: 120, borderLeftWidth: 3, borderRadius: Radius.sm, paddingHorizontal: Spacing.sm, paddingVertical: 2 },
	...
	footer: { flexDirection: "row", alignItems: "flex-end", justifyContent: "flex-end", gap: Spacing.sm, marginTop: Spacing.xs },
	footerWithReactions: { justifyContent: "space-between" },
	reactions: { flexDirection: "row", flexWrap: "wrap", gap: Spacing.xs, flexShrink: 1 },
	reaction: { minWidth: 48, height: 36, minHeight: 36, flexShrink: 0, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4, borderWidth: 1, borderRadius: Radius.full, paddingHorizontal: 6, paddingVertical: 2 },
	...
	reactionCount: { includeFontPadding: false, fontSize: FontSize.xs, lineHeight: 16, fontWeight: FontWeight.semibold, color: Colors.textPrimary },
	meta: { flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 4, alignSelf: "flex-end", flexShrink: 0 },
	ownMeta: { color: Colors.textPrimary },
	metaText: { includeFontPadding: false, fontSize: 10, lineHeight: 14, fontWeight: FontWeight.medium, color: Colors.textMuted },
